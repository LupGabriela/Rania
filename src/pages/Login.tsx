    ...
    <div className="min-h-screen flex items-center justify-center px-4 py-16" style={{ backgroundColor: "#FAF6F0" }}>
      ...
      >
        <div className="text-center mb-10">
          ...
          <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "15px", fontStyle: "italic", color: "#C4846A", marginTop: "4px" }}>Welcome back</p>
        </div>

        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          <div className="flex flex-col gap-1.5">
            <label style={{ fontFamily: "'Jost', sans-serif", fontSize: "12px", fontWeight: 500, letterSpacing: "1px", color: "#3D2B1F", textTransform: "uppercase" }}>Email</label>
            <input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder="your@email.com" required
              className="w-full px-4 py-3 outline-none" style={{ backgroundColor: "#F0E6D3", border: "none", borderRadius: "8px", fontFamily: "'Jost', sans-serif", fontSize: "14px", color: "#3D2B1F" }} />
          </div>

          <div className="flex flex-col gap-1.5">
            <label style={{ fontFamily: "'Jost', sans-serif", fontSize: "12px", fontWeight: 500, letterSpacing: "1px", color: "#3D2B1F", textTransform: "uppercase" }}>Password</label>
            <div className="relative">
              <input type={showPassword ? "text" : "password"} value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} placeholder="••••••••" required
                className="w-full px-4 py-3 pr-12 outline-none" style={{ backgroundColor: "#F0E6D3", border: "none", borderRadius: "8px", fontFamily: "'Jost', sans-serif", fontSize: "14px", color: "#3D2B1F" }} />
              <button type="button" className="absolute right-4 top-1/2 -translate-y-1/2 opacity-50 hover:opacity-80" onClick={() => setShowPassword(!showPassword)} style={{ color: "#3D2B1F" }}>
        ...
        <p className="text-center mt-6" style={{ fontFamily: "'Jost', sans-serif", fontSize: "13px", color: "#3D2B1F", opacity: 0.6 }}>
          ...
          <Link to="/register" style={{ color: "#C9A96E", opacity: 1, textDecoration: "underline" }}>Register</Link>
        </p>
      </motion.div>
    </div>
  );
